		   ...
		   <#list columnDatas as item>
				<#if item.columnKey != 'PRI'>
				 <if test="query.${item.domainPropertyName} != null and query.${item.domainPropertyName} != ''" >
	  		 		/* ${item.columnComment} */
			    	AND ${item.columnName} =  ${"#"}{query.${item.domainPropertyName},jdbcType=${item.jdbcType?upper_case}}
				 </if>
				</#if>
			</#list>
		</trim>
	...
	-->  
	 ...
	 <update id="update" parameterType="Object" >
	  UPDATE   ${tableName}  	/* ${codeName} */
				...
				WHERE
					${columnName} = ${"#{"}${domainPropertyName}${"}"}		/* 序号 */ 
...
${"                  "},${tablesAsName}.${item.columnName}${"                              "?substring(item.columnName?length)}/* ${item.columnComment} */ 
		   ...
		   FROM   ${tableName}       ${tablesAsName}      /* ${codeName} */ 
		   WHERE
					${columnName} = ${"#{"}${domainPropertyName}${"}"}		/* 序号 */ 
	</select>
	...
	<select id="count" resultType="java.lang.Integer"  parameterType="Object">
		SELECT count(*)  FROM  ${tableName}    ${tablesAsName}      /* ${codeName} */ 
		 WHERE 1=1
...
${"                  "},${tablesAsName}.${item.columnName}${"                              "?substring(item.columnName?length)}/* ${item.columnComment} */ 
		...
		FROM  (
SELECT A.*, ROWNUM RN 
FROM (SELECT * FROM ${tableName} where  1=1  <include refid="wherecontation"/>) A WHERE 
${"#"+"{startRow}"}  + ${"#"+"{pageSize}"}
>= ROWNUM) ${tablesAsName}  
WHERE ${tablesAsName}.RN > ${"#"+"{startRow}"} 
